...
module.exports = class Sub extends Signature {
  ...
  static generateKeys ( size=1024, /*type*/ ) {
  
    const key = new NodeRSA({b: size});

    /*
    if ( "string" === typeof type ) { } else {

      type = "pkcs1";
    }
  
    let publicKeyType = type + "-public-pem", privateKeyType = type + "-private-pem";
    */

    let publicKeyType = 'pkcs8-public-pem', privateKeyType = 'pkcs8-private-pem';
